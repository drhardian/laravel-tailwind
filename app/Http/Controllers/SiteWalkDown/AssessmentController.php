...
class AssessmentController extends Controller
{
    ...
    public function __construct(DeviceTypeRepositoryInterface $deviceTypeRepository, AreaRepositoryInterface $areaRepository, OtherAreaRepositoryInterface $otherAreaRepository, CompanyPeopleRepositoryInterface $companyPeopleRepository, HealthRatingRepositoryInterface $healthRatingRepository, ImageRepositoryInterface $imageRepository, AssessmentImageRepositoryInterface $assessmentImageRepository)
    {
        ...
        $this->pageTitle = 'SiteWalkdown Assessment';
    }
    ...
    public function index()
    {
        ...
        session()->forget(['healthLevelSession', 'health_rating_id', 'health_level_color', 'priority_rating_id']);

        return view('sitewalkdown.assessment.index', compact('breadcrumbs', 'title'));
    }
    ...
    public function create()
    {
        ...
        foreach ($otherAreas as $value) {
            $otherarea[] = [$value->otherarea_id => $value->otherarea->title];
        }

        $selectBoxData = [
            'area_sbx' => [$instruction->area_id => $getArea->title],
        ...
        return view('sitewalkdown.assessment.create', compact('breadcrumbs', 'title', 'instruction', 'dateActivity', 'selectBoxData', 'assessmentId'));
    }
        ...
        try {
            ...
            $newProduct = Product::updateOrCreate(
                ...
                array_merge(
                    ...
                    $request->only('device_type_id', 'criticality_level_id', 'tagnum', 'serial_number', 'application', 'name_plate', 'body_mfc', 'body_sn', 'body_model', 'body_material', 'body_size', 'class_rating', 'manual_override', 'end_connection', 'company_id', 'area_id'),
                ),
            );
            ...
            $newAssessment = Assessment::create(
                array_merge(
                    ...
                    $request->only('id', 'instruction_id', 'device_type_id', 'criticality_level_id', 'company_id', 'location_type_id', 'location_detail_id', 'activity_date', 'serial_number', 'application', 'final_recommendation', 'rigging_point_needed', 'rigging_point_available', 'scaffolding_required', 'leak_detection_method', 'value_a', 'value_b', 'value_c', 'value_d', 'passing_detection_result', 'leak_out_value', 'leak_out_result', 'voc_leak_value', 'area_id'),
                ),
            );
            ...
            switch ($deviceType) {
                case 'COV':
                    ...
                    CovProduct::create(array_merge(['product_id' => $newProduct->id], $request->only('insulation', 'leakage_class', 'flow_direction', 'actuator_mfc', 'actuator_sn', 'actuator_model', 'actuator_size', 'fail_position', 'gear_mfc', 'gear_model', 'gear_size', 'positioner_mfc', 'positioner_sn', 'positioner_model', 'communication_protocol', 'instrument_acc', 'instrument_acc_sn', 'info_rating', 'info_plug', 'info_stem', 'info_body', 'info_seat', 'facetoface_dimension')));
                ...
                case 'REG':
                    ...
                    RegProduct::create(array_merge(['product_id' => $newProduct->id], $request->only('orifice_size', 'spring_range', 'spring_color', 'setpoint', 'pilot_mfc', 'pilot_model', 'pilot_springrange', 'valve_size')));
                ...
                case 'CKV':
                    ...
                    CkvProduct::create(array_merge(['product_id' => $newProduct->id], $request->only('valve_design', 'seat_material', 'air_assisted', 'dampener', 'counter_weight')));
                ...
                case 'ISV':
                    ...
                    IsvProduct::create(array_merge(['product_id' => $newProduct->id], $request->only('plug_material', 'seat_material', 'stem_material', 'operator', 'doubleblock_bleed', 'leakage_class', 'actuator_mfc', 'actuator_sn', 'actuator_model', 'actuator_size', 'multi_turn', 'torque_seated', 'quarter_turn', 'position_seated', 'local_control', 'remote_control', 'actuator_ratio', 'fail_position', 'gear_mfc', 'gear_model', 'gear_size', 'instrument_acc', 'instrument_acc_sn', 'info_rating', 'info_plug', 'info_stem', 'info_body', 'info_seat', 'facetoface_dimension')));
                ...
                case 'PRV':
                    ...
                    PrvProduct::create(array_merge(['product_id' => $newProduct->id], $request->only('code', 'inlet', 'inlet_choose', 'outlet', 'outlet_choose', 'orifice_size', 'set', 'capacity', 'pilot_operated', 'choose')));
                ...
                case 'MAV':
                    ...
                    MavProduct::create(array_merge(['product_id' => $newProduct->id], $request->only('leakage_class', 'plug_material', 'seat_material', 'stem_material', 'operator')));
                ...
                default:
                    ...
                    TnkProduct::create(array_merge(['product_id' => $newProduct->id], $request->only('tank_capacity', 'tank_product', 'vapor_pressure', 'specific_gravity', 'avg_storage_temp', 'insulated', 'vents_insulated', 'heated_chilled', 'insulation_reduction_factor', 'max_pump_inrate', 'max_pump_outrate', 'blanketing_gas', 'allowable_tank_moisture', 'allowable_tank_o', 'blanket_gas_supply', 'max_allow_work_press', 'max_allow_work_vac', 'notes')));
    ...
    public function edit(Assessment $assessment)
    {
        ...
        $attributes = (object) [
            'title' => 'Site Walkdown',
            'breadcrumb' => (object) [(object) ['title' => 'Dashboard', 'status' => '', 'previousUrl' => '/'], (object) ['title' => 'Site Walkdown', 'status' => '', 'previousUrl' => route('swd.assessments.index')], (object) ['title' => 'Edit', 'status' => 'active', 'previousUrl' => '']],
        ];
        ...
        foreach ($otherAreas as $value) {
            $otherarea[] = [$value, $this->otherAreaRepository->getTitleById($value)];
        }

        $selectBoxData = [
            'area_sbx' => [$assessment->area_id, $this->areaRepository->getTitleById($assessment->area_id)],
            'otherarea_sbx' => $otherarea,
        ];
        ...
        try {
                ...
                while ($personKey < count($request->persons_name)) {
                    if (!empty($request->persons_name[$personKey])) {
                        if (!empty($request->persons_id[$personKey])) {
                            ...
                            CompanyPerson::where('id', $request->persons_id[$personKey])->update([
                                'name' => $request->persons_name[$personKey],
                                'title' => $request->persons_title[$personKey],
                                'email' => $request->persons_email[$personKey],
                            ]);
            ...
            $assessment->update(
                array_merge(
                    ...
                    $request->only('instruction_id', 'device_type_id', 'criticality_level_id', 'company_id', 'location_type_id', 'location_detail_id', 'activity_date', 'serial_number', 'application', 'final_recommendation', 'rigging_point_needed', 'rigging_point_available', 'scaffolding_required', 'leak_detection_method', 'value_a', 'value_b', 'value_c', 'value_d', 'passing_detection_result', 'leak_out_value', 'leak_out_result', 'voc_leak_value', 'area_id'),
                ),
            );
            ...
            switch ($deviceType) {
                case 'COV':
                    # insert new control valve detail product
                    CovProduct::where('product_id', $assessment->product_id)->update($request->only('insulation', 'leakage_class', 'flow_direction', 'actuator_mfc', 'actuator_sn', 'actuator_model', 'actuator_size', 'fail_position', 'gear_mfc', 'gear_model', 'gear_size', 'positioner_mfc', 'positioner_sn', 'positioner_model', 'communication_protocol', 'instrument_acc', 'instrument_acc_sn', 'info_rating', 'info_plug', 'info_stem', 'info_body', 'info_seat', 'facetoface_dimension'));

                    # insert new control valve assessment
                    CovAssessment::where('assessment_id', $assessment->id)->update([
                        'packing_condition' => $request->valve_condition_1,
                        'packing_condition_level' => $request->health_rating_1,
                        'packing_cause' => implode('|', $request->potensial_cause_1),
                        'packing_recommendation' => implode('|', $request->recommendation_1),
                ...
                case 'REG':
                    ...
                    RegProduct::where('product_id', $assessment->product_id)->update($request->only('orifice_size', 'spring_range', 'spring_color', 'setpoint', 'pilot_mfc', 'pilot_model', 'pilot_springrange', 'valve_size'));

                    # insert new regulator assessment
                    RegAssessment::where('assessment_id', $assessment->id)->update([
                        'body_condition' => $request->valve_condition_1,
                        'body_condition_level' => $request->health_rating_1,
                        'body_cause' => implode('|', $request->potensial_cause_1),
                        'body_recommendation' => implode('|', $request->recommendation_1),
                ...
                case 'CKV':
                    # insert new check valve detail product
                    CkvProduct::where('product_id', $assessment->product_id)->update($request->only('valve_design', 'seat_material', 'air_assisted', 'dampener', 'counter_weight'));

                    # insert new check valve assessment
                    CkvAssessment::where('assessment_id', $assessment->id)->update([
                        'packing_condition' => $request->valve_condition_1,
                        'packing_condition_level' => $request->health_rating_1,
                        'packing_cause' => implode('|', $request->potensial_cause_1),
                        'packing_recommendation' => implode('|', $request->recommendation_1),
                ...
                case 'ISV':
                    # insert new isolation valve detail product
                    IsvProduct::where('product_id', $assessment->product_id)->update($request->only('plug_material', 'seat_material', 'stem_material', 'operator', 'doubleblock_bleed', 'leakage_class', 'actuator_mfc', 'actuator_sn', 'actuator_model', 'actuator_size', 'multi_turn', 'torque_seated', 'quarter_turn', 'position_seated', 'local_control', 'remote_control', 'actuator_ratio', 'fail_position', 'gear_mfc', 'gear_model', 'gear_size', 'instrument_acc', 'instrument_acc_sn', 'info_rating', 'info_plug', 'info_stem', 'info_body', 'info_seat', 'facetoface_dimension'));

                    # insert new isolation valve assessment
                    IsvAssessment::where('assessment_id', $assessment->id)->update([
                        'packing_condition' => $request->valve_condition_1,
                        'packing_condition_level' => $request->health_rating_1,
                        'packing_cause' => implode('|', $request->potensial_cause_1),
                        'packing_recommendation' => implode('|', $request->recommendation_1),
                ...
                case 'PRV':
                    # insert new prv detail product
                    PrvProduct::where('product_id', $assessment->product_id)->update($request->only('code', 'inlet', 'inlet_choose', 'outlet', 'outlet_choose', 'orifice_size', 'set', 'capacity', 'pilot_operated', 'choose'));

                    # insert new prv assessment
                    PrvAssessment::where('assessment_id', $assessment->id)->update([
                        'body_condition' => $request->valve_condition_1,
                        'body_condition_level' => $request->health_rating_1,
                        'body_cause' => implode('|', $request->potensial_cause_1),
                        'body_recommendation' => implode('|', $request->recommendation_1),
                ...
                case 'MAV':
                    # insert new manual valve detail product
                    MavProduct::where('product_id', $assessment->product_id)->update($request->only('leakage_class', 'plug_material', 'seat_material', 'stem_material', 'operator'));

                    # insert new manual valve assessment
                    MavAssessment::where('assessment_id', $assessment->id)->update([
                        'packing_condition' => $request->valve_condition_1,
                        'packing_condition_level' => $request->health_rating_1,
                        'packing_cause' => implode('|', $request->potensial_cause_1),
                        'packing_recommendation' => implode('|', $request->recommendation_1),
                ...
                default:
                    # insert new tank detail product
                    TnkProduct::where('product_id', $assessment->product_id)->update($request->only('tank_capacity', 'tank_product', 'vapor_pressure', 'specific_gravity', 'avg_storage_temp', 'insulated', 'vents_insulated', 'heated_chilled', 'insulation_reduction_factor', 'max_pump_inrate', 'max_pump_outrate', 'blanketing_gas', 'allowable_tank_moisture', 'allowable_tank_o', 'blanket_gas_supply', 'max_allow_work_press', 'max_allow_work_vac', 'notes'));

                    # insert new tank assessment
                    TnkAssessment::where('assessment_id', $assessment->id)->update([
                        'main_product' => $request->main_product,
                        'other_product' => $request->other_product,
                    ]);
                    break;
    ...
    public function showRowsOnTable()
    {
        ...
        $query = Assessment::with(['product:id,tagnum', 'instruction:id,instruction_num', 'company:id,name', 'priorityRating:id,title,color', 'locationDetail:id,title']);
