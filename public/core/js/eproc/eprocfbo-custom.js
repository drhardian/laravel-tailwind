...
openForm = (url) => {
    modalShowAndReset();
    $('.modal-title').text('New Input FBO');
    $('#form_url').val(url);
}
...
editRecord = (url) => {
    modalShowAndReset();
    $('.modal-title').text('Edit Input FBO');
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            var type_processOptions = new Option(response.dropdown.type_process, response.dropdown.type_process, true, true);
            $('#type_process').append(type_processOptions).trigger('change');
            var tkdn_requiredOptions = new Option(response.dropdown.tkdn_required, response.dropdown.tkdn_required, true, true);
            $('#tkdn_required').append(tkdn_requiredOptions).trigger('change');
            var required_categoryOptions = new Option(response.dropdown.required_category, response.dropdown.required_category, true, true);
            $('#required_category').append(required_categoryOptions).trigger('change');
            var eprocmethodOptions = new Option(response.dropdown.eprocmethod, response.dropdown.eprocmethod, true, true);
            $('#eprocmethod').append(eprocmethodOptions).trigger('change');
            var statusfboOptions = new Option(response.dropdown.statusfbo, response.dropdown.statusfbo, true, true);
            $('#statusfbo').append(statusfboOptions).trigger('change');
